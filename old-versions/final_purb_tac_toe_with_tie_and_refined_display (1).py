...
def display_board(board, player_name, show_labels=True):
    """Display the board dynamically, creating labels for empty spaces if needed."""
    print(f"{player_name}:")
    label_count = 1
    for row in board:
        row_display = []
        if any(cell != " " for cell in row) or show_labels:
            for cell in row:
                if cell == " ":
                    if show_labels:
                        if label_count <= 9:
...
def check_tie(board):
    """Check for a tie condition (cat's game)."""
...
def main():
    ...
    while True:
        ...
        display_board(initial_board, 'purb', show_labels=False)
        if check_win(initial_board, 'o'):
            print("purb wins!")
            break
        if check_tie(initial_board):
            print("cat’s game!")
            break
